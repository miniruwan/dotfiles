...
*/
#HotIf WinActive("cdb-", )  ; If the window title starts with "cdb-"" Eg title: "cdb-test-au01-test-custprf - Microsoft Azure and 7 more pages"
        ...
        ; Example cursor position: "parti<<CURSOR_POSITION>>tionKey": "WorkplaceLink-AP-4AFFCF25-A69C-4E80-AD81-71316884684A",
{
        ...
        IB := InputBox("Enter the partition key you want to filter on", "Partition Key", "w420 h120", A_Clipboard), partitionKey := IB.Value, ErrorLevel := IB.Result="OK" ? 0 : IB.Result="CANCEL" ? 1 : IB.Result="Timeout" ? 2 : "ERROR"
        if ErrorLevel
            return
        ...
        Send("{Raw}WHERE c.partitionKey = `"" partitionKey "`" ORDER BY c._ts DESC")
        Send("{Tab}")
